...
export function useTabTearOff(
  onTearOff:
    | ((screenX: number, screenY: number) => Promise<string | null>)
    | undefined,
) {
  const rpc = useRpc()
...
/**
 * A single tab button — the canonical look shared between the orchestrator's
 * flat LeafPane and any plugin-owned tab strip (e.g. the Spaces window view).
 */
export function PaneTab({
  ...
  const canClose = Boolean(onClose || onCloseQuiet)
  const handleClose = () => {
    if (hasUserMessages && onClose) onClose()
    else if (onCloseQuiet) onCloseQuiet()
    else onClose?.()
  }
...
/**
 * The horizontal strip container that hosts a row of `PaneTab`s. Shared
 * between LeafPane and plugin-owned window views.
 */
      ...
      <div className="flex items-center min-w-0 flex-1 overflow-x-auto [scrollbar-width:thin] [scrollbar-color:var(--color-neutral-400)_transparent]">
        {tabIds.map((tabId) => (
          <PaneTab
            ...
            onClose={onCloseTab ? () => onCloseTab(tabId) : undefined}
            onCloseQuiet={
              onCloseTabQuiet ? () => onCloseTabQuiet(tabId) : undefined
            }
            onContextMenu={
              onTabContextMenu ? () => onTabContextMenu(tabId) : undefined
            }
            onTearOff={
              onTabTearOff
                ? (screenX, screenY) => onTabTearOff(tabId, screenX, screenY)
                : undefined
            }
            tabRef={
              tabRefs
