...
import { CheckIcon, ChevronDownIcon, CopyIcon, FolderOpenIcon, FolderSyncIcon } from "lucide-react"
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          <svg width="17" height="17" viewBox="0 0 24 24" className="-rotate-90">
            <circle
              cx="12" cy="12" r={radius}
              fill="none" stroke="#c4c4c4" strokeWidth={stroke}
            />
            <circle
              cx="12" cy="12" r={radius}
              fill="none"
              stroke={fraction > 0.9 ? "#ef4444" : fraction > 0.7 ? "#f59e0b" : "#737373"}
              strokeWidth={stroke}
      ...
      <PopoverContent side="top" align="center" className="w-auto px-3 py-2 text-xs">
        <div className="flex items-center gap-1 text-neutral-600">
          <span className="font-medium text-neutral-900">{formatTokens(used)}</span>
          <span>/</span>
...
function ComposerCwdInner({
  ...
  const agents = useDb((root) => root.plugin.kernel.agents) as AgentRow[] | undefined
  const agentConfigs = useDb((root) => root.plugin.kernel.agentConfigs) as AgentConfigRow[] | undefined
  ...
  const usage = useMemo(() => {
    ...
    for (let i = events.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        return { used: e.data.update.used as number, size: e.data.update.size as number }
      }
    }
    return null
  }, [events])
  ...
  const workspaces = useDb((root) => root.plugin.kernel.workspaces) as
    ...
    | undefined
  const activeWorkspaceId = useDb((root) => root.plugin.kernel.activeWorkspaceId) as
    | string
    | undefined
  ...
  const agentCwd = (agent?.metadata?.cwd as string | undefined) ?? workspace?.cwd
          ...
          {showCwd ? (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="min-w-[160px] text-xs">
                <DropdownMenuItem
                  className="text-xs"
                  onClick={() => navigator.clipboard.writeText(agentCwd!)}
                >
